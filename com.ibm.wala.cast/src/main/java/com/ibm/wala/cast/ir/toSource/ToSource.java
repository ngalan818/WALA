...
public class ToSource {
  ...
  public static void toJava(IR ir, IClassHierarchy cha, TypeInference types) {
    ...
    MutableIntSet mergedValues = IntSetUtil.make();
    Map<SSAInstruction, Set<SSAPhiInstruction>> condPhis = HashMapFactory.make();
    IntegerUnionFind mergePhis = new IntegerUnionFind(ST.getMaxValueNumber());
    ...
    Map<Set<Pair<SSAInstruction, ISSABasicBlock>>, Set<ISSABasicBlock>> regions =
        HashMapFactory.make();
    cdg.forEach(
        node -> {
    ...
    Map<Pair<SSAInstruction, ISSABasicBlock>, Set<Set<SSAInstruction>>> regionChunks =
        HashMapFactory.make();
    regions
        .entrySet()
        .forEach(
    ...
    class TypeInferenceContext implements CAstVisitor.Context {
    ...
    class RegionTreeNode {
      SSAInstruction r;
      ISSABasicBlock l;
      private final Map<SSAInstruction, Map<ISSABasicBlock, RegionTreeNode>> children =
          HashMapFactory.make();

      private RegionTreeNode(SSAInstruction r, ISSABasicBlock l) {
        this.r = r;
        this.l = l;
        bbs()
            .forEach(
                                ...
                                .forEach(
                                    k -> {
                                      if (k.fst == bb.getLastInstruction()) {
                                        ...
                                        children
                                            .get(k.fst)
                                            .put(k.snd, new RegionTreeNode(k.fst, k.snd));
                                      }
                                    });
                          });
                });
      ...
      public CAstNode toCAst() {
        ...
        chunks.stream()
            ...
            .forEach(
                c -> {
                  elts.add(new ToCAst(c, new TypeInferenceContext()).processChunk());
                });
        orderByInsts(
                chunks,
                ToSource::defsSet,
            ...
            .forEach(
                chunkInsts -> {
                  if (!phiChunk(chunkInsts) && !gotoChunk(chunkInsts)) {
                    elts.add(new ToCAst(chunkInsts, new TypeInferenceContext()).processChunk());
                  }
                });
        chunks.stream()
      ...
      class ToCAst {
        ...
        class Visitor implements AstPreInstructionVisitor {
          ...
          private List<CAstNode> handleBlock(
              Set<Set<SSAInstruction>> loopChunks, RegionTreeNode lr) {
            List<CAstNode> block =
                StreamSupport.stream(
                        orderByInsts(
                            ...
                            .spliterator(),
                        false)
                    .map(c -> lr.new ToCAst(c, new TypeInferenceContext()).processChunk())
          ...
          private void visitAbstractInvoke(SSAAbstractInvokeInstruction inst) {
        ...
        private ToCAst(Set<SSAInstruction> chunk, TypeInferenceContext c) {
          this.chunk = chunk;
          this.c = c;
        }
    ...
    RegionTreeNode root = new RegionTreeNode(cfg.getNode(1).getLastInstruction(), cfg.getNode(2));
    ...
    class ToJavaVisitor extends CAstVisitor<TypeInferenceContext> {
    ...
    toJava.visit(ast, new TypeInferenceContext(), toJava);
  }
}
